                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-1.5 flex-wrap">
                    ...
                    {plan.containerNumber && !plan.isDelivered && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleSyncContainerEta(plan);
                        }}
                        disabled={isSyncingPlanContainer === plan.container}
                        className="px-2 py-1 bg-sky-50 hover:bg-sky-100 text-sky-800 border border-sky-300 rounded-lg text-[11px] font-bold transition flex items-center space-x-1"
                        title="Sync live status and arrival ETA from carrier API"
                      >
                        <RefreshCw className={`w-3 h-3 text-sky-600 ${isSyncingPlanContainer === plan.container ? 'animate-spin' : ''}`} />
                        <span>{isSyncingPlanContainer === plan.container ? 'Syncing...' : 'Update Status'}</span>
                      </button>
                    )}
                    ...
                    {plan.isDelivered && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleUnmarkDeliver(plan);
                        }}
                        className="px-2 py-1 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300 rounded-lg text-[11px] font-bold transition flex items-center space-x-1"
                        title="Revert container delivery status back to Active / In Transit (correct mistake)"
                      >
                        <RotateCcw className="w-3 h-3 text-amber-600" />
                        <span>Make Undelivered</span>
                      </button>
                    )}
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                ...
                <div className="flex items-center space-x-2 flex-wrap gap-y-2">
                  ...
                  {activePlan.containerNumber && !activePlan.isDelivered && (
                    <button
                      onClick={() => handleSyncContainerEta(activePlan)}
                      disabled={isSyncingPlanContainer === activePlan.container}
                      className="px-3.5 py-2 bg-sky-50 hover:bg-sky-100 text-sky-800 border border-sky-300 rounded-xl text-xs font-bold shadow-md transition flex items-center space-x-1.5"
                      title="Sync live status and arrival ETA from shipping line API"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 text-sky-600 ${isSyncingPlanContainer === activePlan.container ? 'animate-spin' : ''}`} />
                      <span>{isSyncingPlanContainer === activePlan.container ? 'Syncing...' : 'Sync / Update Status'}</span>
                    </button>
                  )}
                  ...
                  {activePlan.isDelivered && (
                    <button
                      onClick={() => handleUnmarkDeliver(activePlan)}
                      className="px-3.5 py-2 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300 rounded-xl text-xs font-bold shadow-md transition flex items-center space-x-1.5"
                      title="Revert container delivery status back to Active / In Transit (correct mistake)"
                    >
