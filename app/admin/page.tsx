...
export default function AdminDashboardPage() {
  ...
  const [apiKeyUpdateStatus, setApiKeyUpdateStatus] = useState<{ type: 'success' | 'error'; message: string } | null>(null);

  const handleUpdateApiKey = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newApiKeyInput.trim()) {
      setApiKeyUpdateStatus({ type: 'error', message: 'Please enter a JSONCargo API key' });
      return;
    }
    setIsUpdatingApiKey(true);
    setApiKeyUpdateStatus(null);
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || 'Failed to verify API key with JSONCargo');
      }
      setApiKeyUpdateStatus({
        type: 'success',
        message: data.message || 'JSONCargo API key verified and updated successfully!',
      });
      setNewApiKeyInput('');
      fetchApiStats();
    } catch (err: any) {
      setApiKeyUpdateStatus({ type: 'error', message: err.message || 'API key update failed' });
    } finally {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between px-5 py-3.5 bg-slate-50 border-b border-slate-200 gap-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                {failedSyncList.length > 0 && (
                  <button
                    type="button"
                    onClick={handleClearSyncAlerts}
                    className="px-2.5 py-1 text-xs font-bold rounded-lg bg-rose-50 border border-rose-200 text-rose-700 hover:bg-rose-100 flex items-center space-x-1"
                    title="Purge all logged sync errors once resolved"
                  >
